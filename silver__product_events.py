...
enriched AS (
    SELECT
            *,
            ...
            DATE_TRUNC('month', CAST(event_timestamp AS DATE)) AS event_ts_month,

            CASE
                WHEN event_name ILIKE '%login%' THEN 'AUTH'
                WHEN event_name ILIKE '%pipeline%' OR event_name ILIKE '%deal%' THEN 'PIPELINE'
                WHEN event_name ILIKE '%activity%' OR event_name ILKE '%call%' OR event_name ILIKE '%email%' THEN 'ACTIVITY'
...
print("silver__product_events table created successfully.")
